...
- Provision foundational components:
  - Main Database, PostgreSQL (`main-db`)
...
- Implement the Auth Service in the `auth-service` container using .NET 8.
...
## Phase 3: Web Service
- Develop the API server in the `web-service` container using .NET 8.
...
- Create a .NET worker running in the `background-worker` container.
...
## Phase 5: Client App
- Build a ChatGPT-style interface in the `client-app` container.
...
- Deploy Prometheus and Grafana in the `monitoring` container.
